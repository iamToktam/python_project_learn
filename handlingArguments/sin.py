...
class TerminalScribe:
    def __init__(self, canvas, color='red', mark='*', trail='.', pos=(0, 0), framerate=.05):
        self.canvas = canvas
        self.trail = trail
        self.mark = mark
...
class PlotScribe(TerminalScribe):
    def walkSine(self, function, steps):
        for x in range(steps):
            y = function(x)  # Get the y-value for the sine wave
            if 0 <= round(y) < self.canvas._y:  # Ensure y is within canvas bounds
                self.draw((x, round(y)))

# Define the sine function
def sine(x):
    return 5 * math.sin(x / 4) + 15  # Adjusted sine wave to fit the canvas

# Initialize the canvas and scribe
canvas = CanvasAxis(30, 30)  # A 30x30 canvas
scribe = PlotScribe(canvas, color='green', mark='*', trail='.')

# Walk along the sine wave
scribe.walkSine(sine, steps=30)  # Walk 30 steps along the sine wave
